function dx = partFunctionNucCatE(x,K_N_E,K_C2_N, cost)
...
lambda_C2_E = 0.01 *(1+ (K_N_E*((N/(AN*cell_vol))/((N/(AN*cell_vol))+Km_N_E))));
end
if lambda_C2_E >1
    lambda_C2_E = 1;
end
lambda_C2_S = 0.0505*(1-lambda_C2_E);
lambda_C2_FA = 0.3798*(1-lambda_C2_E);
lambda_C2_AA = 0.5697*(1-lambda_C2_E);
...
delta_C2 =  (AA_1/(AN*cell_vol) * (CO2_sink)/(CO2_sink + Km) * (K_C2 + K_C2 * (1-(K_N_E * cost)) * K_C2_N * N/(AN*cell_vol)))*dt;
