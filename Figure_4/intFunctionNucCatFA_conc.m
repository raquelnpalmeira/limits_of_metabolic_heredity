function mean_N_conc_eq = intFunctionNucCatFA_conc(K_N_FA,K_C2_N, ndays,div, cost)
%Run model with nucleotides intFunctionC2(1e-4,1e-2,1e5, 1e4, 1e4, 1e4, 1, 864000,1)
...
for time=1:tmax
    t=t+1;
    dx = partFunctionNucCatFA(x(t,:),K_N_FA,K_C2_N, cost);
    dx_vector(t,:) = partFunctionNucCatFA(x(t,:),K_N_FA,K_C2_N, cost); %create a vector with all dxs
    ...
    if div == 1
        if x(t+1,3)>=2e8%e15
        t=t+1;
        disp(['Cell Divided!!',num2str(x(t,3))])%,num2str()]);
        x(t+1,1)= (x(t,1))/2;
        x(t+1,2)= (x(t,2))/2;
        x(t+1,3)= (x(t,3))/2;
        x(t+1,4)= (x(t,4))/2;
