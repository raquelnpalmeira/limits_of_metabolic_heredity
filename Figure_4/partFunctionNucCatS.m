function dx = partFunctionNucCatS(x,K_N_S,K_C2_N, cost)
...
lambda_C2_S = 0.05 * (1 + (K_N_S * ((N/(AN*cell_vol))/((N/(AN*cell_vol))+Km_N_S))));
end
if lambda_C2_S > 1 
    lambda_C2_S = 1;
end
lambda_C2_FA = 0.3958*(1-lambda_C2_S);
lambda_C2_E = 0.0105*(1-lambda_C2_S);
lambda_C2_AA = 0.5937*(1-lambda_C2_S);
...
delta_C2 =  (AA_1/(AN*cell_vol) * (CO2_sink)/(CO2_sink + Km) * (K_C2 + K_C2 * (1-(K_N_S * cost)) * K_C2_N * N/(AN*cell_vol)))*dt;
