function [divnperday,mean_N_conc_eq] = intFunctionNucCatC2_parsweep(K_C2_N,K_N_N,ndays,div)
...
for time=1:tmax
    t=t+1;
    dx = partFunctionNucCatC2_parsweep(K_C2_N,K_N_N,x(t,:));
    dx_vector(t,:) = partFunctionNucCatC2_parsweep(K_C2_N,K_N_N,x(t,:)); %create a vector with all dxs
    
    %make sure no x is negative
    if find(x(t,:)+round(dx,0)<0) > 0
        x(t+1,:)=x(t,:)+round(dx,0);
        x(t+1,find(x(t,:)+round(dx,0)<0)) = 0; 
    else 
        x(t+1,:)=x(t,:)+round(dx,0);
        
    end
    
    %Cell  Division: everytime that the number of fatty acids doubles, divide the number of
    %every molecule by 2
    if div == 1
        if x(t+1,3)>=2e8
        t=t+1;
